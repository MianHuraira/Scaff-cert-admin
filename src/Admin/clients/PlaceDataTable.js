...
const PlaceDataTable = ({data, columns}) => {
    // ** State
    ...
    const handleFilter = e => {
        ...
        if (value.length) {
            updatedData = data.filter(item => {
                const startsWith =
                    item.PlaceName.toLowerCase().startsWith(value.toLowerCase()) ||
                    item.PlaceType.toLowerCase().startsWith(value.toLowerCase()) ||
                ...
                const includes =
                    ...
                    item.PlaceType.toLowerCase().includes(value.toLowerCase()) ||
    ...
    return (
        <Card>
            ...
            <div className='react-dataTable mb-2'>
                <DataTable
                    onRowClicked={() => history.push('subplace')}
                    noHeader
                    responsive
                    pagination
